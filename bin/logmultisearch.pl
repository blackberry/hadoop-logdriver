...
while (@ARGV > 0) {
  ...
  if ($arg eq '--') {
    ...
    if ($arg eq '-v') {
      ...
      print STDERR "AND searching selected.\n";
      push @D_options, " -Dlogdriver.search.and=true";
    }
...
my $mkdir_cmd = "$DFS_MKDIR $tmp/strings";

# If the $strings is a dir, copy all files from that dir.  If it's a file,
# copy the file.  Else, assume it's the only string.
my $hput_cmd = '';
if (! -e $strings) {
  ## mktemp, create the file, copy it up and delete it.
  $hput_cmd = 'dir=`mktemp -d` '
        . '&& echo ' . escape($strings) . ' > ${dir}/strings '
        . '&& ' . $DFS_PUT . ' ${dir}/strings ' . $tmp . '/strings/strings '
            . "$redirects 1>&2"
        . '&& rm -rf ${dir}' . "$redirects 1>&2";
}
elsif (-d $strings) {
  ## put directory contents
  $hput_cmd = $DFS_PUT . ' ' . $strings . '/* ' . $tmp . '/strings'.
        "$redirects 1>&2"; 
}
else {
  # put file
...
my $mr_cmd = "$HADOOP_JAR $LOGDRIVER_HOME/$LOGDRIVER_HDEPLOY "
        . "com.rim.logdriver.util.MultiSearchByTime"
        . " " . $mr_opts
        . " " . escape("$tmp/strings")
        . " " . escape($dc)
        . " " . escape($service)
...
my $rm_tmp_cmd = "$DFS_RMR $tmp &>/dev/null";

$quiet or print STDERR "Running: $mkdir_cmd\n";
(0 == system $mkdir_cmd)
  || die $!;

$quiet or print STDERR "Running: $hput_cmd\n";
(0 == system $hput_cmd)
  || die $!;
